...
### Option 3: Use with Other Tools (Cursor, Aider, Windsurf, Gemini CLI, Codex, OpenCode, Kimi Code)
...
- **Codex** — `SKILL.md` per agent → `$CODEX_HOME/skills/` (default `~/.codex/skills/`)
...
Each agent is converted to a Codex skill (`SKILL.md`) and installed to
...
Then use a skill in Codex by name:
```
Use the agency-frontend-developer skill to review this component.
```

After installation, restart Codex so newly installed skills are loaded.
...
- [x] Multi-tool integration scripts (Claude Code, GitHub Copilot, Antigravity, Gemini CLI, Codex, OpenCode, OpenClaw, Cursor, Aider, Windsurf, Qwen Code, Kimi Code)
